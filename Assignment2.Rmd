---
title: "Assignment2"
author: "Yifan Yin, Aditi Nagaraj Nallan"
date: "26/01/2021"
output:
  html_document:
    toc: true
...
# my answer
conda install -n Assignment2 sambamba
```
...
# my answer
fastqc -o /home/yyin input_iPSC_SRA66_subset_1_fastqc.gz
fastqc -o /home/yyin input_iPSC_SRA66_subset_2_fastqc.gz
...
# my answer
There is no much difference observed in the FastQC results in read1 and read2. Both them show a good quality, as many of the modules are green tick, only one showing orange triangle. The basic statistics of read1 and read2 are all the same (same total sequences, same sequence length, and samt %GC). The biggest difference is in the Per base sequence quality module. In read1, almost all bases have a good quality score. In contrast, some bases in read2 have lower quality calls which fall into orange and red region. In other modules, the results are almost the same.
## -
## -
...
# my answer
bowtie2 -x hg38_bowtie2_index -1 /usr/local/share/data/assignment_2/input_iPSC_SRA66_subset_1.fastq.gz -2 /usr/local/share/data/assignment_2/input_iPSC_SRA66_subset_2.fastq.gz -S /home/yyin/input_data.sam
...
# my answer
samtools view -b -h input_data.sam > input_data.bam 
...
samtools view input_data.bam | cut -f1,3,4 | head -5
#?# Use the documentation format (# at the beginning of the line) to include the output of the command above (one per line): - 0.5pt

# my answer
1. SRR12694366.1000000     chr3    46631700
2. SRR12694366.1000000     chr3    46631857
3. SRR12694366.10000029    chr3    77719001
4. SRR12694366.10000029    chr3    77718984
5. SRR12694366.10000035    chr3    40015576
...
samtools view input_data.sorted.bam | cut -f1,3,4 | head -5
#?# Use the documentation format (# at the beginning of the line) to include the output of the command above (one per line): 0.5 pt

# my answer
1. SRR12694366.7794671     chr12   133198776
2. SRR12694366.7794671     chr12   133198776
3. SRR12694366.26500791    chr3    9988
4. SRR12694366.33624963    chr3    10013
5. SRR12694366.33624963    chr3    10013

#?# What changed between the two files? Describe what is sambamba sort doing. - 1 pt

# my answer
Before sorting, the files are sorted based on read ID. After sorting, the files are sorted based on genomic position on each chromosome.
...
# my answer
sambamba view -h -f bam -F "[XS] == null and not unmapped and not duplicate" input_data.sorted.bam > unique_read.bam
...
# my answer
Before filtering, there are 4415764 reads. After filtering, there are 3859882 unique reads. 

samtools view -c -F 4 input_data.sorted.bam
samtools view -c -F 4 unique_read.bam
```
...
```{bash, eval=FALSE}
./runTheseJobsSerially.sh echo pipeline.txt
#?# what happened? Enter your answer below - 1 pt
# my answer
The results returned the sample ID and the name of input datasets
...
./runTheseJobsSerially.sh ./fastqToFilteredBam.sh pipeline.txt
#?# What happened? Enter your answer below - 1 pt

# my answer
The results returned:
running pipeline for iPSC_input
Performing fastqc of sample iPSC_input with the following fastqs:
/usr/local/share/data/assignment_2/input_iPSC_SRA66_subset_1.fastq.gz
/usr/local/share/data/assignment_2/input_iPSC_SRA66_subset_2.fastq.gz

This is the messages that are written in the script. These messages will occur when we first run the pipelines on the IPSC_input datasets.
...
This message occurred becasue we have already run the pipelines on the IPSC_input. In the script, it will check whether the datasets are already run before. If yes, it will return the message of 'Already performed fastqc of iPSC_input'.
...
I deleted the if else statement which checks whether this is the first time to run the datasets.
#!/bin/bash
set -e # this makes the whole script exit on any error.
#fill these variables with the arguments given to this script
sample=$1
...
then    
        ...
        fastqc -o /home/yyin /usr/local/share/data/assignment_2/$fq1
        fastqc -o /home/yyin /usr/local/share/data/assignment_2/$fq2
        
        echo fastqc done
        
        # perform alignment of the input dataset using bowtie2
        echo Performing alignment of sample $sample with the following fastqs:
        ls /usr/local/share/data/assignment_2/$fq1 /usr/local/share/data/assignment_2/$fq2
        
        bowtie2 -x /usr/local/share/indexes/hg38_bowtie2_index -1 /usr/local/share/data/assignment_2/$fq1 -2 /usr/local/share/data/assignment_2/$fq2 -S /home/yyin/input_data.sam
        ...
        samtools view -b -h /home/yyin/input_data.sam > /home/yyin/input_data.bam 
        ...
        sambamba sort /home/yyin/input_data.bam
        ...
        sambamba view -h -f bam -F "[XS] == null and not unmapped and not duplicate" /home/yyin/input_data.sorted.bam > /home/yyin/unique_read.bam
